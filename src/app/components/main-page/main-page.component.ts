...
export class MainPageComponent implements OnInit, OnDestroy{
  ...
  public addToDo() {    
    ...
    this.service.addToDo$(body as ToDos)
    .pipe(takeUntil(this.unSubscribe$$))
    .subscribe((data) => {
      console.log(data);
    });
  ...
  public editToDo(id: number) {
    ...
    const dialogReg = this.dialog.open(EditComponentDialogComponent, { data: this.editToDoInput?.todo});
    ...
    .subscribe(result => {
      console.log(result);
      if(result) {
        this.service.editToDo$(result, this.editToDoInput?.id as number)
        .pipe(takeUntil(this.unSubscribe$$))
        .subscribe((data) => {
          console.log(data);
        ;});
        this.todos.map((data) => {if(data.id == this.editToDoInput?.id){data.todo = result}})
      }
    })
  }
